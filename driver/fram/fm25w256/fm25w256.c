...
#define SPI_BUS     SPI_BUS_MCBSP_A
#elif CONFIG_SPI_FM25W256_MCBSPB
#define SPI_BUS     SPI_BUS_MCBSP_B
#endif
...
static FM25W256_RET_T FM25W256_transact(Bool isWrite, UInt8 * pBuf, UInt16 len)
{
    ...
    if(isWrite) {
        ...
        ret_spi = SPI_transact_8bit(SPI_BUS, &OpWrEn, 1, &bResult, sem_sync, FM25W256_CS_select);
        if(SPI_OK != ret_spi) {
    ...
    ret_spi = SPI_transact_8bit(SPI_BUS, pBuf, len, &bResult, sem_sync, FM25W256_CS_select);
    if(SPI_OK != ret_spi) {
        GateMutexPri_leave(fm25w256_mutex, key);
        return FM25W256_ERROR;
    }
